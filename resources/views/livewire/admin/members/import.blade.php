...
>
    <div class="flex items-center gap-4 mb-6">
        ...
        <flux:heading size="xl">Import Members</flux:heading>
    </div>
    ...
    @if ($activeImport)
        <flux:card class="max-w-2xl space-y-4">
            ...
            @elseif ($activeImport->status === 'completed')
                ...
                <flux:button variant="primary" :href="route('admin.members.index')" wire:navigate>View Members</flux:button>
        ...
        <flux:card class="max-w-2xl space-y-6">
            <div>
                ...
                <flux:text class="mt-1 text-zinc-500">
                    Upload the members CSV. Required columns:
                    <code class="text-sm font-mono bg-zinc-100 dark:bg-zinc-800 px-1 rounded">Membership No</code> and
                    <code class="text-sm font-mono bg-zinc-100 dark:bg-zinc-800 px-1 rounded">ContactName</code>.
                </flux:text>
            </div>
            ...
            @if ($showPreview && count($previewRows) > 0)
                <div>
                    <flux:heading size="sm" class="mb-2">Preview (first {{ count($previewRows) }} of {{ number_format($totalRows) }} rows)</flux:heading>
                ...
                <div class="flex gap-2">
                    <flux:button variant="primary" wire:click="startImport" wire:loading.attr="disabled">
                        <span wire:loading.remove wire:target="startImport">Queue Import ({{ number_format($totalRows) }} rows)</span>
                        <span wire:loading wire:target="startImport">Queuing...</span>
                    </flux:button>
                    <flux:button variant="ghost" :href="route('admin.members.index')" wire:navigate>Cancel</flux:button>
                </div>
            @endif
        </flux:card>
    @endif
