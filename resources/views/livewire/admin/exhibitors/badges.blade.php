<div class="space-y-4">
    ...
    <flux:card>
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-start min-w-0 gap-4">
                <div class="shrink-0">
                    {!! $qrSvg !!}
                </div>
                <div class="min-w-0 pt-1 space-y-1">
                    ...
                    <flux:text class="font-mono text-xs text-zinc-400 dark:text-zinc-500">
    ...
    <flux:modal wire:model="showAddModal" name="add-member" class="w-full max-w-md">
        <div class="space-y-4">
            ...
            <flux:file-upload wire:model="memberPhoto" label="Photo" badge="Optional"
                description="Max 2MB. JPG, PNG, or WEBP." :error="$errors->first('memberPhoto')">
                <flux:file-upload.dropzone heading="Drop photo here or click to browse"
                    text="JPG, PNG, or WEBP, up to 2MB" with-progress inline />
            </flux:file-upload>

            @if ($memberPhoto)
                <flux:file-item :heading="$memberPhoto->getClientOriginalName()" :image="$memberPhoto->temporaryUrl()"
    ...
    <flux:modal wire:model="showEditModal" name="edit-member" class="w-full max-w-md">
        <div class="space-y-4">
            ...
            <flux:file-upload wire:model="memberPhoto" label="Photo"
                description="Optional — leave blank to keep existing. Max 2MB. JPG, PNG, or WEBP."
                :error="$errors->first('memberPhoto')">
                <flux:file-upload.dropzone heading="Drop photo here or click to browse"
                    text="JPG, PNG, or WEBP, up to 2MB" with-progress inline />
            </flux:file-upload>
