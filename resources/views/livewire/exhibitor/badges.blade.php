<div x-data="{
                        ...
                        <button type="button" x-on:click="selectedMemberId = {{ $member->id }}"
                            ...
                            @if ($member->photo)
                                <img src="{{ route('exhibitor.badges.photo', [$booking, $member]) }}"
                                    alt="{{ $member->name }}" class="w-9 h-9 rounded-full object-cover shrink-0">
                            @else
                                <div
    ...
    <flux:modal wire:model="showAddModal" name="add-member" class="w-full max-w-md">
        <div class="space-y-4">
            ...
            <flux:file-upload wire:model="memberPhoto" label="Photo" description="Max 2MB. JPG, PNG, or WEBP."
