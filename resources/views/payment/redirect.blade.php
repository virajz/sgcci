        ...
        <flux:card class="p-8">
            ...
            <div class="flex items-center justify-between mb-8">
                ...
                @if ($booking?->exhibition?->logo_url)
                    <img src="{{ $booking->exhibition->logo_url }}" alt="{{ $booking->exhibition->title }} Logo" class="h-auto max-w-24">
                @endif
            </div>
            ...
            {{-- Booking Information --}}
            <div class="space-y-4">
                <flux:heading size="lg" class="mb-4">Booking Summary</flux:heading>

                <div class="flex items-center justify-between py-3 border-b border-zinc-200 dark:border-zinc-700">
                    <flux:text class="text-zinc-600 dark:text-zinc-400">Booking Code</flux:text>
                    <flux:text class="font-semibold text-black dark:text-white">{{ $booking->booking_code }}</flux:text>
                </div>
                ...
                <div class="flex items-center justify-between py-3 border-b border-zinc-200 dark:border-zinc-700">
                    <flux:text class="text-zinc-600 dark:text-zinc-400">Brand / Dealership</flux:text>
                ...
                @if ($booking->discount_percentage > 0)
                    <div class="flex items-center justify-between py-3 border-b border-zinc-200 dark:border-zinc-700">
                        <flux:text class="text-zinc-600 dark:text-zinc-400">Discount
                            ({{ number_format((float) $booking->discount_percentage, 2) }}%)</flux:text>
                        <flux:text class="font-semibold text-green-600 dark:text-green-400">
